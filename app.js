...
// ========== Helper URL PRIM (encodé via proxy) ==========
...
function parseStop(data) {
  ...
  return visits.map(v => {
    ...
    const rawDest =
      (Array.isArray(dd) ? dd[0]?.value : dd?.value) ||
...
// ========== RER (Joinville – Ligne A) ==========
async function renderRer() {
  const cont = document.getElementById("rer-body");
  if (!cont) return;
  cont.textContent = "Chargement…";

  const data = await fetchJSON(primUrl("/stop-monitoring", {
    MonitoringRef: STOP_IDS.RER_A,
    LineRef: LINES_SIRI.RER_A
  }));
...
async function renderBusForStop(stopId, bodyId, trafficId) {
  ...
  if (!stopId) {
    cont.innerHTML = `<div class="traffic-sub alert">⚠️ Aucun arrêt configuré</div>`;
    return;
  }
  ...
  // Regrouper par ligne puis destination
  const byLine = {};
  for (const v of visits) (byLine[v.lineId] ||= []).push(v);
  ...
  for (const [lineId, rows] of sortedLines) {
    ...
    const byDest = {};
    for (const r of rows) (byDest[r.dest] ||= []).push(r);
    ...
    for (const [dest, list] of sortedDest) {
      ...
      list
        ...
        .slice(0, 4)
        .forEach(it => timesEl.insertAdjacentHTML("beforeend", formatTimeBox(it)));
...
// ========== Trajet optimal Joinville → Hippodrome ==========
async function computeBestRouteJoinville() {
  ...
  try {
    ...
    velibOK = ((st?.mechanical_bikes || 0) + (st?.ebike_bikes || 0)) > 0;
  } catch {}
...
// ========== Météo ==========
const WEATHER_CODES = {
  0: { emoji: "☀️", text: "Grand soleil" }, 1: { emoji: "🌤️", text: "Ciel dégagé" }, 2: { emoji: "⛅", text: "Éclaircies" },
  3: { emoji: "☁️", text: "Ciel couvert" }, 45: { emoji: "🌫️", text: "Brouillard" }, 48: { emoji: "🌫️", text: "Brouillard givrant" },
  51: { emoji: "🌦️", text: "Bruine légère" }, 53: { emoji: "🌦️", text: "Bruine" }, 55: { emoji: "🌧️", text: "Forte bruine" },
  56: { emoji: "🌧️", text: "Bruine verglaçante" }, 57: { emoji: "🌧️", text: "Bruine verglaçante" },
  61: { emoji: "🌦️", text: "Pluie faible" }, 63: { emoji: "🌧️", text: "Pluie" }, 65: { emoji: "🌧️", text: "Pluie forte" },
  66: { emoji: "🌧️", text: "Pluie verglaçante" }, 67: { emoji: "🌧️", text: "Pluie verglaçante" },
  71: { emoji: "🌨️", text: "Neige légère" }, 73: { emoji: "🌨️", text: "Neige" }, 75: { emoji: "❄️", text: "Neige forte" },
  77: { emoji: "❄️", text: "Grésil" }, 80: { emoji: "🌦️", text: "Averses" }, 81: { emoji: "🌧️", text: "Averses" },
  82: { emoji: "🌧️", text: "Forte averse" }, 85: { emoji: "🌨️", text: "Averses de neige" }, 86: { emoji: "❄️", text: "Averses de neige" },
  95: { emoji: "⛈️", text: "Orages" }, 96: { emoji: "⛈️", text: "Orages grêle" }, 99: { emoji: "⛈️", text: "Orages grêle" }
};
...
async function refreshTransitTraffic() {
  ...
  // 1) PRIM general-message pour RER A + Bus 77/201
  try {
    const impacted = [];
    let appended = false;
    ...
    if (rerInfo) {
      if (infosRer.length) {
        const txt = cleanText(
          infosRer[0]?.Content?.Message?.[0]?.MessageText?.[0]?.value ||
          infosRer[0]?.Content?.Message?.[0]?.MessageText?.value || ""
        );
        rerInfo.style.display = "block";
        rerInfo.textContent = txt || "Information trafic disponible";
        rerInfo.className = "traffic-sub alert";
        impacted.push({ label: "RER A", detail: txt || "Perturbation" });
      } else {
        rerInfo.style.display = "block";
        rerInfo.textContent = "Trafic normal";
        rerInfo.className = "traffic-sub ok";
      }
    }
    ...
    for (const [lbl, lineRef] of [["77", LINES_SIRI.BUS_77], ["201", LINES_SIRI.BUS_201]]) {
      ...
      if (infos.length) {
        const txt = cleanText(
          infos[0]?.Content?.Message?.[0]?.MessageText?.[0]?.value ||
          infos[0]?.Content?.Message?.[0]?.MessageText?.value || ""
        );
        div.className = "traffic-sub alert";
        div.innerHTML = `<strong>Bus ${lbl}</strong> — ${txt || "Perturbation"}`;
        impacted.push({ label: `Bus ${lbl}`, detail: txt || "Perturbation" });
      } else {
        div.className = "traffic-sub ok";
        div.textContent = `Bus ${lbl} — Trafic normal`;
      }
    ...
    return; // Succès PRIM → on sort
  } catch (e) {
    console.warn("PRIM general-message indisponible, fallback RATP…", e);
  }

  // 2) Fallback RATP si PRIM indisponible
  try {
...
function startLoops() {
  ...
  if (STOP_IDS.JOINVILLE_BUSES) {
    setInterval(() => renderBusForStop(STOP_IDS.JOINVILLE_BUSES, "bus-joinville-body", "bus-joinville-traffic"), 60000);
  }
...
(async function init() {
  ...
  await Promise.allSettled([
    renderRer(),
    STOP_IDS.JOINVILLE_BUSES ? renderBusForStop(STOP_IDS.JOINVILLE_BUSES, "bus-joinville-body", "bus-joinville-traffic") : Promise.resolve(),
